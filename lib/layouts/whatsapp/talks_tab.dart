...
class TalksTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView.separated(
        itemBuilder: (context, index) => TalkTabItem(chat: messageData[index],),
      ...
      trailing: Column(
        children: <Widget>[
          ...
          Container(width: 25, height: 25, decoration: BoxDecoration(
              color: chat.unreadMessagesCount > 0 ? Colors.green : Colors.transparent,
              shape: BoxShape.circle
          ),
            alignment: Alignment.center,
            child: Text(chat.unreadMessagesCount.toString(), textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold),),
          ),
        ],
      ),
      title: Text(chat.name, style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold),),
      subtitle: Row(
        children: <Widget>[
          _buildIcon(chat.messageStatus),
          SizedBox(width: 6),
          Flexible(child: Text(chat.message, overflow: TextOverflow.ellipsis ,style: TextStyle(color: Colors.black),)),
          SizedBox(width: 6),
          chat.isMuted ? Icon(Icons.volume_off, color: Colors.grey) : Container(),
        ],
      ),
    );
  }
